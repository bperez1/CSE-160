...
let a_UV; // UV attribute
let u_Sampler0; // Texture sampler uniform
let u_ViewMatrix; // View matrix uniform
let u_ProjectionMatrix; // Projection matrix uniform
let u_whichTexture; // Uniform to switch between textures
...
function initTextures() {
    initTexture('lib/textures/sky.jpg', 0); // Load sky texture to texture unit 0
    initTexture('lib/textures/dirt.jpg', 1); // Load ground texture to texture unit 1
    initTexture('rock.jpg', 2); // Load rock texture to texture unit 2
    initTexture('water.jpg', 3); // Load water texture to texture unit 3
}
...
function tick() {
    updateCameraBasedOnKeys(camera); // Update camera position based on key states
...
function main() {
    ...
    // Bind the camera's matrices to shader program
...
var keyStates = {}; // Keep track of the pressed keys
function initEventHandlers(camera) {
    ...
    canvas.onmousedown = function(event) {
        ...
        if (event.ctrlKey && event.button === 0) { 
            ...
            if (controlEnabled) {
                // Lock the mouse to the canvas and hide the cursor
            ...
            // Alt key is held, global rotation
            mouseDown = true;
            rotateMode = true;
        } else if (event.button === 0) {
            // Normal left click, FPS-style movement
            mouseDown = true;
            rotateMode = false;
        }
    
        // Shift+Click for triggering poke animation
    ...
    function updatePosition(e) {
        ...
        let sensitivityFactor = 0.3; // Adjust this value to increase or decrease sensitivity

        camera.panHorizontal(-movementX * sensitivityFactor);
        camera.panVertical(-movementY * sensitivityFactor);
...
function updateCameraBasedOnKeys(camera) {
    ...
    // Ensure the camera matrices are updated only if any relevant key is pressed
...
function drawHamster() {
    ...
    bottomLeftLegParts.forEach(part => part.textureNum = 0);
    ...
    topRightLegParts.forEach(part => part.textureNum = 0);
