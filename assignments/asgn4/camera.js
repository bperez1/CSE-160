class Camera {
  constructor(canvas) {
    ...
    this.near = 0.1;
    this.far = 100;
    this.eye = new Vector3([5, 3, 5]); // Initial eye position
    this.at = new Vector3([3, 3, 3]); // Initial at position
  ...
  panLeft(angle) {
    angle = angle * 15;
    let rotationMatrix = new Matrix4().setRotate(
      angle,
      this.up.elements[0],
      this.up.elements[1],
      this.up.elements[2]
    ...
    let x = forward.elements[0],
      y = forward.elements[1],
      z = forward.elements[2];
  ...
  panVertical(angle) {
    let verticalAxis = Vector3.cross(
      new Vector3(this.at.elements).sub(this.eye),
      this.up
    ).normalize(); // Cross product for perpendicular axis
